...
const basicTests = [
  ...
  'SELECT col as col1 FROM t1 WHERE t1.id = 1 GROUP BY col1',
  'SELECT col as col1 FROM t1 T1 WHERE T1.id = 1 GROUP BY col1',
  'SELECT col as col1 FROM t1 T1 LEFT JOIN t2 T2 ON T1.id = T2.id WHERE',
  ...
  "select count(*)\\nfrom works_common wc join foo ON foo.id = wc.id",
  "select count(*)\\nfrom works_common wc inner join foo ON foo.id = wc.id",
  "select count(*)\\nfrom works_common wc inner join hierarchy hwc on wc.id > 5",
  "select count(*)\\nfrom works_common wc inner join hierarchy hwc on wc.id = hwc.parentid",
  "select count(*)\\nfrom works_common wc inner join hierarchy hwc on wc.id = hwc.parentid and hwc.primarytype = 'workTermGroup'",
  "select count(*)\\nfrom works_common wc inner join hierarchy hwc on (wc.id = hwc.parentid and hwc.primarytype = 'workTermGroup')",
  "select count(*)\\nfrom works_common wc inner join hierarchy hwc on (wc.id = hwc.parentid and hwc.primarytype = 'workTermGroup')",
