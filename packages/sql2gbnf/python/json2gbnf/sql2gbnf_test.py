...
def test_it_calls_sql2gbnf(schema, expected):
    result = sql2gbnf(schema)
    assert result.split("\n")[: len(expected)] == expected


def test_it_calls_sql2gbnf_with_fixed_order():
    schema = {
        "type": "object",
        "additionalProperties": False,
        "properties": {
    ...
    result = sql2gbnf(schema, fixed_order=True, whitespace=0)
    expected = [
...
def test_it_calls_sql2gbnf_without_whitespace():
    schema = {
        "type": "object",
        "additionalProperties": False,
        "properties": {
    ...
    result = sql2gbnf(schema, whitespace=0)
    expected = [
        f"""xa ::= {QUOTE_KEY} "number" {QUOTE_KEY} {COLON_KEY} {NUMBER_KEY}""",
        f"""xb ::= {QUOTE_KEY} "street_name" {QUOTE_KEY} {COLON_KEY} {STRING_KEY}""",
        f"""xc ::= {QUOTE_KEY} "Street" {QUOTE_KEY} | {QUOTE_KEY} "Avenue" {QUOTE_KEY} | {QUOTE_KEY} "Boulevard" {QUOTE_KEY}""",
